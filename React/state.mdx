...
1. 이해
   ...
   State는 react에서 데이터를 저장할 수 있는 특수한 변수이다. 값이 바뀌었을 시, 이것이 화면에 반영되도록 연결된 변수가 state이다. 따라서 JS에서 기본적으로 제공하는 기본적인 변수인 `const`, `let` ~~, `var`~~ 과는 다르게 state의 값이 변하면 그 값을 참조한 component들이 자동으로 다시 rendering된다.
