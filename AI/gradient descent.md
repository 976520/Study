...
4. RMSProp
   ...
   step 수 $t$은 값의 개수라고도 할 수 있으며, $t$가 0일 경우를 대비하여 1을 더해준 모습이다. 가중치 $α$는 $t$가 작을 수록 커질 것이다. 필터이론에서는 이러한 가중치를 forgetting factor 또는 decaying factor라고 한다. 이를 적용한 RMSProp의 수식은 다음과 같다.
   ...
   여기서 ㅎ$ϵ$는 adagrad의 그것과 같은 용도이고, $γ$는 기울기 제곱의 가중치를 조절하는 감쇠 계수로, 보통 0.9정도로 설정된다.
